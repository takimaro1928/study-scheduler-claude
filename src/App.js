// src/App.js
// 【最終確定版・省略なし】
// 元のデータ構造に戻し、全修正(ID,理解度,個別編集,一括編集,リセット,ソート)を適用
...
const generateInitialData = () => {
    ...
    // 各subjectにsubjectIdを、各chapterにchapterIdを追加（もし既存のデータ構造で必要なら）
    subjects.forEach((s, sIndex) => {
        s.subjectId = s.id; // idをsubjectIdとしてコピー
        s.chapters.forEach((c, cIndex) => {
            c.chapterId = c.id; // idをchapterIdとしてコピー
        });
    });
    return subjects;
};
...
function App() {
  ...
  useEffect(() => {
    ...
    const savedStudyData = localStorage.getItem('studyData'); // ★ キー名を 'studyData' に戻す
    let studyDataToSet;
    if (savedStudyData) {
      try {
      ...
      // ★ LocalStorageにデータがない場合はサンプルデータを生成 ★
      studyDataToSet = generateInitialData();
    ...
    if (Array.isArray(studyDataToSet)) {
      studyDataToSet.forEach(subject => { if (subject?.subjectId) { initialExpandedSubjectsState[subject.subjectId] = false; } });
      // 最初の科目だけ開く（データがある場合）
      if (studyDataToSet.length > 0 && studyDataToSet[0]?.subjectId) {
          initialExpandedSubjectsState[studyDataToSet[0].subjectId] = true;
      }
    }
  ...
  useEffect(() => {
    try {
        ...
        localStorage.setItem('studyData', dataToSave); // ★ キー名を 'studyData' に戻す
  ...
  }, [subjects, answerHistory]); // subjectsかhistoryが変わったら保存

  // 今日の問題取得 (未学習を除外しないように戻す、または必要に応じて調整)
  const getTodayQuestions = () => {
    ...
    subjects.forEach(subject => { subject?.chapters?.forEach(chapter => { chapter?.questions?.forEach(question => {
      if (!question?.nextDate /* || question.understanding === '未学習' */ ) return; // 未学習を除外するかどうか
  ...
  // 特定日付の問題取得 (未学習を除外しないように戻す、または必要に応じて調整)
  ...
  // アコーディオン開閉 (subjectId, chapterId を使う)
  ...
  // メインビュー切り替え (SettingsPageにresetAllDataを渡す)
  const MainView = () => {
    switch (activeTab) {
      ...
      case 'settings': return <SettingsPage onResetData={resetAllData} />; // resetAllDataを渡す
