...
// 問題生成関数 (コメントプロパティを追加 & understanding修正 & IDゼロパディング修正)
...
// 初期データ生成関数 (過去問ID生成ロジック変更)
const generateInitialData = () => {
    // ★ 科目略称マッピングを追加
    const pastExamSubjectPrefixMap = {
        "企業経営理論": "企経",
        "運営管理": "運営",
        "経済学・経済政策": "経済",
        "経営情報システム": "情報",
     ...
     const subjects = [
        ...
        // ★★★ 科目 7 (過去問題集) の chapters を動的に生成 ★★★
        {
            id: 7, name: "過去問題集", chapters: [
                // 元のデータをテンプレートとして用意 (年度、科目名、問題数を抽出するため)
                { id: 701, name: "企業経営理論 令和6年度", questionCount: 40 },
                { id: 702, name: "企業経営理論 令和5年度", questionCount: 37 },
                { id: 703, name: "企業経営理論 令和4年度", questionCount: 37 },
                { id: 704, name: "企業経営理論 令和3年度", questionCount: 38 },
                { id: 705, name: "企業経営理論 令和2年度", questionCount: 36 },
                { id: 706, name: "運営管理 令和6年度", questionCount: 41 },
                { id: 707, name: "運営管理 令和5年度", questionCount: 37 },
                { id: 708, name: "運営管理 令和4年度", questionCount: 39 },
                { id: 709, name: "運営管理 令和3年度", questionCount: 41 },
                { id: 710, name: "運営管理 令和2年度", questionCount: 42 },
                { id: 711, name: "経済学・経済政策 令和6年度", questionCount: 22 },
                { id: 712, name: "経済学・経済政策 令和5年度", questionCount: 22 },
                { id: 713, name: "経済学・経済政策 令和4年度", questionCount: 21 },
                { id: 714, name: "経済学・経済政策 令和3年度", questionCount: 23 },
                { id: 715, name: "経済学・経済政策 令和2年度", questionCount: 22 },
                { id: 716, name: "経営情報システム 令和6年度", questionCount: 23 },
                { id: 717, name: "経営情報システム 令和5年度", questionCount: 25 },
                { id: 718, name: "経営情報システム 令和4年度", questionCount: 24 },
                { id: 719, name: "経営情報システム 令和3年度", questionCount: 25 },
                { id: 720, name: "経営情報システム 令和2年度", questionCount: 25 },
                { id: 721, name: "経営法務 令和6年度", questionCount: 24 },
                { id: 722, name: "経営法務 令和5年度", questionCount: 21 },
                { id: 723, name: "経営法務 令和4年度", questionCount: 22 },
                { id: 724, name: "経営法務 令和3年度", questionCount: 20 },
                { id: 725, name: "経営法務 令和2年度", questionCount: 22 },
                { id: 726, name: "中小企業経営・政策 令和6年度", questionCount: 11 },
                { id: 727, name: "中小企業経営・政策 令和5年度", questionCount: 22 },
                { id: 728, name: "中小企業経営・政策 令和4年度", questionCount: 22 },
                { id: 729, name: "中小企業経営・政策 令和3年度", questionCount: 22 },
                { id: 730, name: "中小企業経営・政策 令和2年度", questionCount: 22 }
            ].map(chapterInfo => {
                const yearMatch = chapterInfo.name.match(/令和(\d+)年度/);
                // 科目名は「 令和」の手前までを取得
                const subjectMatch = chapterInfo.name.match(/^(.+?)\s+令和/);

                if (yearMatch && subjectMatch) {
                    // 年度を R + 2桁数字 (R06, R05 など) に変換
                    const year = `R${yearMatch[1].padStart(2, '0')}`;
                    const subjectName = subjectMatch[1];
                    // マップから略称を取得、なければ科目名から記号を除去
                    const prefixBase = pastExamSubjectPrefixMap[subjectName] || subjectName.replace(/[・/]/g, '');
                    // ★ 新しいIDプレフィックスを生成 ('Q' を含める)
                    const prefix = `過去問-${year}-${prefixBase}-Q`;
                    return {
                        id: chapterInfo.id, // 章IDはそのまま
                        name: chapterInfo.name, // 章名はそのまま
                        // ★ 新しいプレフィックスで generateQuestions を呼び出す
                        questions: generateQuestions(prefix, 1, chapterInfo.questionCount)
                    };
                } else {
                    // もし章名から年度や科目が取得できない場合のフォールバック (念のため)
                    console.warn(`Could not parse year/subject from chapter name: ${chapterInfo.name}`);
                    // 既存の古いID形式を使うか、エラー処理をするかなど検討
                    // ここでは空の questions 配列を返す例
                    return { id: chapterInfo.id, name: chapterInfo.name, questions: [] };
                }
            })
        } // chapters 配列ここまで
    ]; // subjects 配列ここまで
                    ...
                    if (q) {
                        if (q.lastAnswered && typeof q.lastAnswered === 'string') {
                            ...
                            if (!isNaN(parsedDate)) q.lastAnswered = parsedDate;
                             ...
                             // 文字列でも Date でもない場合、過去の形式かもしれないので Date に変換試行
                            const parsedDate = new Date(q.lastAnswered);
                            if (!isNaN(parsedDate)) q.lastAnswered = parsedDate;
                            else q.lastAnswered = null; // それでもダメなら null
                        }
