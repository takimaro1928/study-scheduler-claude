// src/App.js (ユーザー提供コードベースに必要な3点のみ修正)
...
// generateQuestions, generateInitialData はユーザー提供のものを維持
function generateQuestions(prefix, start, end) {
    ...
    for (let i = start; i <= end; i++) {
        ...
        questions.push({
            id: `${prefix}-${i}`,
            number: i,
            correctRate: Math.floor(Math.random() * 100), // 初期値変更: 0 -> random
            lastAnswered: new Date(today.getTime() - Math.floor(Math.random() * 30) * 24 * 60 * 60 * 1000), // 初期値変更: null -> random date
            nextDate: nextDate.toISOString(), // ★ ISO文字列で初期化推奨
            interval: ['1日', '3日', '7日', '14日', '1ヶ月', '2ヶ月'][Math.floor(Math.random() * 6)], // 初期値変更: '1日' -> random
            answerCount: Math.floor(Math.random() * 10), // 初期値変更: 0 -> random
            understanding: ['理解○', '曖昧△', '理解できていない×'][Math.floor(Math.random() * 3)], // 初期値変更: '未解答' -> random
            previousUnderstanding: null, // これはOK
        });
    }
    return questions;
}
...
function App() {
  ...
  // ★ 一括編集用日付 state を App で管理（ユーザー提供コードに合わせる）
  const [bulkEditSelectedDate, setBulkEditSelectedDate] = useState(new Date());

  // 初期データロード useEffect (ユーザー提供のもの、Date復元追加)
  useEffect(() => {
        ...
        dataToSet.forEach(subject => {
            subject.chapters.forEach(chapter => {
                chapter.questions.forEach(q => {
                    ...
                    // previousUnderstanding は recordAnswer で設定されるので復元不要
                });
            });
        });
  ...
  // データ保存 useEffect (ユーザー提供のもの、Date→ISO文字列変換追加)
  useEffect(() => {
    if (subjects.length > 0) {
      ...
      const dataToSave = JSON.stringify(subjects, (key, value) => {
          ...
          // それ以外の Date オブジェクトも念のため文字列化 (ただし他の場所で使っていない前提)
  ...
  // getTodayQuestions (ユーザー提供のもの、日付堅牢化済み)
        ...
        chapter.questions.forEach(question => {
          ...
          try {
            ...
            if (nextDate.getTime() === todayTime) {
              questions.push({ ...question, subjectName: subject.name, chapterName: chapter.name });
            }
  ...
  // getQuestionsForDate (ユーザー提供のもの、日付堅牢化済み)
  ...
  const recordAnswer = (questionId, isCorrect, understanding) => {
    setSubjects(prevSubjects => {
        ...
        chapters: subject.chapters.map(chapter => ({
          ...
          questions: chapter.questions.map(q => {
            if (q.id === questionId) {
              ...
              return {
                ...question, lastAnswered: today.toISOString(), nextDate: nextDate.toISOString(),
                interval: newInterval, answerCount: (question.answerCount || 0) + 1,
                understanding: understanding, previousUnderstanding: previousUnderstanding,
      ...
      return newSubjects; // ユーザー提供コードにはなかった return を追加（mapの動作上はなくても動くが明示的に）
    });
  };
  ...
  const handleQuestionDateChange = (questionId, newDate) => {
    setSubjects(prevSubjects => {
      ...
      if (isNaN(targetDate.getTime())) { console.error("無効なドロップ先の日付:", newDate); return prevSubjects; }
      targetDate.setHours(0, 0, 0, 0);
      ...
      const newSubjects = prevSubjects.map(subject => ({
        ...subject, chapters: subject.chapters.map(chapter => ({
          ...chapter, questions: chapter.questions.map(q => {
            if (q.id === questionId) { console.log(`問題 ${questionId} の日付を ${formatDate(targetDate)} に変更`); return { ...q, nextDate: targetDateString }; }
            return q; }) })) }));
      return newSubjects;
    });
  };
          ...
          questions: chapter.questions.map(q => {
            if (q.id === questionData.id) {
              ...
              try {
                ...
                // 他のフィールド(interval, understanding, correctRate, answerCount等)も上書きされる
  ...
  // saveBulkEdit はユーザー提供のもの
  const saveBulkEdit = (date) => {
     const targetDate = new Date(date);
     if (isNaN(targetDate.getTime())) { console.error("無効な一括編集日付:", date); return; }
     targetDate.setHours(0, 0, 0, 0);
     const targetDateString = targetDate.toISOString();
     setSubjects(prevSubjects => {
       const newSubjects = prevSubjects.map(subject => ({
    ...
    switch (activeTab) {
      ...
      case 'schedule':
        return <ScheduleView
               ...
               />;
      case 'all':
        // ★★★ 3. RedesignedAllQuestionsView に props を追加 ★★★
        return <RedesignedAllQuestionsView
          ...
          setEditingQuestion={setEditingQuestion} // 編集モーダルを開く
          setBulkEditMode={setBulkEditMode}
          bulkEditMode={bulkEditMode}
          selectedQuestions={selectedQuestions}
          setSelectedQuestions={setSelectedQuestions} // 一括編集用に App の state 更新関数を渡す
          saveBulkEdit={saveBulkEdit}               // 一括編集の保存
          formatDate={formatDate}                   // ★ Props 追加
          toggleQuestionSelection={toggleQuestionSelection} // ★ Props 追加
          // 一括編集用カレンダーの日付 state も渡す
          selectedDate={bulkEditSelectedDate}       // ★ Props 追加
          setSelectedDate={setBulkEditSelectedDate} // ★ Props 追加
        />;
      case 'trends':
     ...
     <div className="min-h-screen bg-gray-50">
       ...
       <div>
          <div className="bg-indigo-600 p-4 sm:p-6">
            <div className="max-w-7xl mx-auto"> {/* ここも 5xl -> 7xl に変更 */}
           ...
           <MainView />
            ...
            {editingQuestion && (
              <QuestionEditModal
                ...
                onSave={saveQuestionEdit} // App の save 関数を渡す
