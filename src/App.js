// src/App.js
// 【最終確認用・完全版・省略なし】
// 必要な関数定義(generateInitialData, naturalSortCompareなど)を全て含む
...
const generateInitialData = () => {
    ...
    subjects.forEach((s) => { s.subjectId = s.id; s.chapters.forEach((c) => { c.chapterId = c.id; }); }); // IDの整合性のため
    return subjects;
};
...
function App() {
        ...
        studyDataToSet.forEach(subject => {
            subject?.chapters?.forEach(chapter => {
                chapter?.questions?.forEach(q => {
                    if (q) {
                        if (q.lastAnswered && !(q.lastAnswered instanceof Date)) { const parsedDate = new Date(q.lastAnswered); q.lastAnswered = !isNaN(parsedDate) ? parsedDate : null; }
  ...
  const getQuestionsForDate = (date) => {
    ...
    if (!Array.isArray(subjects)) return questions;
    subjects.forEach(subject => { subject?.chapters?.forEach(chapter => { chapter?.questions?.forEach(question => { if (!question?.nextDate) return; try { const nextDate = new Date(question.nextDate); if (isNaN(nextDate.getTime())) return; nextDate.setHours(0, 0, 0, 0); if (nextDate.getTime() === targetTime) { const subjName = subject?.subjectName ?? '?'; const chapName = chapter?.chapterName ?? '?'; questions.push({ ...question, subjectName: subjName, chapterName: chapName }); } } catch(e) { console.error("Error parsing nextDate in getQuestionsForDate:", e, question); } }); }); });
    return questions.sort((a,b) => naturalSortCompare(a.id, b.id));
  };
  ...
  // ★ 日付フォーマット ★
   ...
   const resetAllData = () => {
       ...
       if (window.confirm("本当にすべての学習データ（解答履歴含む）をリセットしますか？\nこの操作は元に戻せません。")) {
           try {
               localStorage.removeItem('studyData'); // 'studyData' を削除
               localStorage.removeItem('studyHistory');
               console.log("LocalStorageのデータを削除しました。");
               alert("学習データをリセットしました。ページをリロードして初期データを再生成します。");
               window.location.reload(); // リロードしてuseEffectでサンプルデータを生成
